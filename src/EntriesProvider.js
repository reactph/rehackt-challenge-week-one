...
const EntriesProvider = ({ children }) => {
    ...
    const shuffledEntries = useRef(currentSlug ? getFirstThenShuffle(currentSlug) : arrayShuffle(entries))

    return (
        <EntriesContext.Provider value={{ entries: shuffledEntries.current, currentSlug }}>
            {children}
